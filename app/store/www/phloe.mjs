...
const pRe = /({{[\S]+?}})/g,
 ...
 R = dir(import.meta.url), Rl = R.length,
 wFu = u => (u = String(u)).startsWith(R) ? u.substring(Rl) : undefined,
 da = "do-active", Hcss = [`<link rel="stylesheet" href="`, , `">`], Hjs = [`<script type="text/javascript" src="`, , `"></script>`], Hmjs = [`<script type="module" src="`, , `"></script>`],
 M = {
  StringAr, dir, wwwDir: R,
  w3UrlFromUrl: wFu,
  w3DirOfUrl: u => dir(wFu(u)),
  urlFromW3Url: u => R + (u.startsWith(S) ? S0 : S) + u,
  doActive: da, buildTag: { css: u => { Hcss[1] = u; return Hcss.join(S0) }, js: u => { Hjs[1] = u; return Hjs.join(S0) }, mjs: u => { Hmjs[1] = u; return Hmjs.join(S0) } }
 }
export default M
...
if (typeof window !== "undefined") {
 ...
 W.addEventListener("load", () => aDT(doc))
 W.phloemjs = M
 Object.assign(StringAr.prototype, { toDom: function () { wR(this.HTMLgetRequireList(L, L1)); const nl = NfS(this.toString()); nl.forEach(aDT); return nl }, HTMLrequire: wR, nodesFromString: NfS, activeDomTree: aDT, appendInto: function (p) { this.toDom().forEach(_a, p) }, insertBefore: function (n) { this.toDom().forEach(_iB, n) } })
}